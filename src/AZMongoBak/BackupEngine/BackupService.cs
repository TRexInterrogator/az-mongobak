            ...
            try {
                if (this._db.BackupInfos is not null && this._db.ConnectionProfiles is not null) {
                    ...
                    if (connection is not null) {
                        job.AppendOut($"Loaded connection profile: {connection.oid} {connection.displayname}");

                        // Create our directory
                        Directory.CreateDirectory(this._out_dir);
                        job.AppendOut($"Created local directory {this._out_dir}");
                        job.AppendOut("Running mongodump..");
                        ...
                        job.AppendOut(process.StandardError.ReadToEnd());
                        job.AppendOut(process.StandardOutput.ReadToEnd());
                        job.AppendOut("Creating zip from dump..");

                        // Zip out directory
                        var zip_file = $"{this._out_dir}.zip";
                        ZipFile.CreateFromDirectory(this._out_dir, zip_file);
                        job.AppendOut($"Compressed dump: {zip_file}");

                        // Delete local dump
                        Directory.Delete(this._out_dir, true);
                        job.AppendOut("Deleted local dump files");

                        // Upload to blob-storage
                        job.AppendOut("Starting blob storage upload..");
                        var blob_name = await this.UploadBlobAsync(zip_file);

                        if (!string.IsNullOrEmpty(blob_name)) {
                            // Create backup db record
                            job.AppendOut("Successfully uploaded to blob storage");
                            this._bak_info.backups.Add(new Backup(blob_name));
                        }
                        else {
                            // Job will be ended and saved in finally
                            job.AppendOut("Blob upload failed");
                        }
                    }
                }
            }
                ...
                job.AppendOut($"Backup engine error: {ex.Message}");
            }
            finally {
                // End job and update job details
                if (this._db.BackupInfos is not null) {
                    job.EndJob();
                    var job_index = this._bak_info.jobs.FindIndex(e => e.job_oid == job.job_oid);

                    if (job_index >= 0) {
                        this._bak_info.jobs[job_index] = job;
                        await this._db.BackupInfos.ReplaceOneAsync(e => e.oid == this._bak_info.oid, this._bak_info);
                    }
                }
